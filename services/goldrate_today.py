...
async def scrape_gold_rates(url: str) -> dict:
    ...
    async with async_playwright() as p:
        ...
        try:
            # Use plain ASCII output so Windows console encodings don't crash the scraper.
            print(f"Loading: {url}")
...
def display_results(data: dict) -> None:
    ...
    if not rates:
        print("No gold rates found. Try running again or check website structure.")
        return
    ...
    for carat in ["24K", "22K", "18K"]:
        if carat in rates:
            rate = rates[carat]
            # Avoid rupee symbol; keep it ASCII-friendly.
            print(f"{carat:<10} {rate['purity']:<10} {rate['currency']} {rate['price_per_gram']:,}{' ':<8} {rate['unit']:<10}")
        else:
